...
public class MqttServerImpl implements MqttServer {
    ...
    public Future<MqttServer> listen(int port, String host) {
        ...
        log.info("""
                Starting MQTT server on port {} and host {}
                """);
        server.exceptionHandler(t -> {
            if (h2 != null) {
                h2.handle(t);
            } else {
                log.error("Exception in MQTT server", t);
            }
        });
        server.connectHandler(so -> {
            ...
            soi.exceptionHandler(t -> {
                if (h2 != null) {
                    h2.handle(t);
                } else {
                    log.error("Exception in MQTT server connection", t);
                }
            });
        });
    ...
    private void initChannel(ChannelPipeline pipeline) {
        ...
        pipeline.addAfter("handler", "exp", new ChannelDuplexHandler() {
            @Override
            public void exceptionCaught(ChannelHandlerContext ctx, Throwable cause) throws Exception {
                log.error("捕获到异常: " + cause.getMessage());
                cause.printStackTrace(); // 打印详细的异常堆栈信息
                ctx.close(); // 关闭连接
            }
        });
    }
